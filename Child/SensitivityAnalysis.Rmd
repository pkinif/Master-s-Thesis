...
To ensure the robustness of the main findings of the previous section I carry out a Sensitivity Analysis which investigates how the variation in the output of equation \ref{EconometricModel} can be attributed to variations of its input [@Pianosi2016]. First, equation \ref{EconometricModel} had been re-estimated using a lagged variables of two years, such as:

\begin{equation}
\centering
\begin{aligned}
Y_{it+2} & = & \alpha + \beta_{1} SPL_{it} + \beta_{2} STC_{it} + \beta_{3} A_{it} + \beta_{4} CaP_{it} \\
...
Estimators of equation \ref{EconometricModelLag2} are reported in table \ref{Lag2}. Based on the results of both BPLM and F tests, estimators had been estimated with the *pooled OLS estimation*. Except for the estimator of WaP in Model (4) which loose its significativity, findings stay the same than in section: *"[Results]"*. 

Second, I use GS assigned to each company of the NGR as an alternative proxy for CEP. GS is based on a weighted average of KPI of the ranking. Concretely, it means that equation \ref{EconometricModel} becomes:
...
Given the pvalue of both BPLM and F tests, Model (4) had been estimated with the *pooled OLS estimation* while Model (5) had been estimated with the *fixed effect estimation*. Results are reported in table \ref{GreenScoreResults} and confirm findings of the previous section. More precisely, it shows that a 1% increase of GS increases the long-term CFP (+ 0.669) and the short-term CFP (+ 0.051) of companies.

Hence, the sensitivity analysis supports that CEP does have a significant and positive effect on CFP, no matter the time horizon (short-term and long-term), and is stronger with a long-term perspective than a short-term perspective. `R` script of this section is available in *"[Appendix D: Sensitivity Analysis - R Script]"*.
