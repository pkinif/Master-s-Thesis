...
Lag1_Roa_Outliers<-data.frame(read.csv2("DataBase/Lag_1.csv", sep = ";",stringsAsFactors=FALSE, header = TRUE )) #ROA
Lag1_Roe_NoOutliers<-data.frame(read.csv2("DataBase/NoOutliers/Lag1_Roe_NoOutliers.csv", sep = ";",stringsAsFactors=FALSE, header = TRUE ))
Lag1_Tobin_NoOutliers<-data.frame(read.csv2("DataBase/NoOutliers/Lag1_Tobin_NoOutliers.csv", sep = ";",stringsAsFactors=FALSE, header = TRUE ))


#I select only variables of my model

DB_Lag1_Model <- Lag1_Roa_Outliers [,c("ROA", "ROE", "TobinsQ", "EnergyProductivity",
...
Roa <- plm(ROA ~ SustainabilityPayLink + SustainableThemedCommitment + AuditScore + EnergyProductivity + CarbonProductivity + WaterProductivity + WasteProductivity + Leverage + NetMargin + FirmSize + Industry , model = "pooling", data = Lag1_Roa_Outliers, index = c("Companies", "YearFinancialIndicator"))

TobinsQ <- plm(TobinsQ ~ SustainabilityPayLink + SustainableThemedCommitment + AuditScore + EnergyProductivity + CarbonProductivity + WaterProductivity + WasteProductivity + Leverage + NetMargin + FirmSize + Industry , model = "pooling", data = Lag1_Tobin_NoOutliers, index = c("Companies", "YearFinancialIndicator"))

Roe <- plm(ROE ~ SustainabilityPayLink + SustainableThemedCommitment + AuditScore + EnergyProductivity + CarbonProductivity + WaterProductivity + WasteProductivity + Leverage + NetMargin + FirmSize + Industry , model = "pooling", data = Lag1_Roe_NoOutliers, index = c("Companies", "YearFinancialIndicator"))
