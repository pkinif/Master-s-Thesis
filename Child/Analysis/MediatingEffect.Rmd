...
10-04-18: D'après les résultats, pas de miateur effect.... maintenant je ne pense pas que mon code soit correct.
...
```{r}
Xnames <- c("SustainabilityPayLink", "AuditScore")

m.med_Energy <- multimed(outcome = "ROA", treat = "SustainableThemedCommitment", med.main = "EnergyProductivity", med.alt = c("CarbonProductivity", "WaterProductivity", "WasteProductivity"), covariates = Xnames, data = DB_Lag1, sims = 1000 )

m.med_Carbon <- multimed(outcome = "ROA", treat = "SustainableThemedCommitment", med.main = "CarbonProductivity", med.alt = c("EnergyProductivity", "WaterProductivity", "WasteProductivity"), covariates = Xnames, data = DB_Lag1, sims = 1000 )

summary(m.med_Energy)
summary(m.med_Carbon)
