...
# M3_pvcm <- pvcm(TobinsQ ~ EnergyProductivity + CarbonProductivity + WaterProductivity + WasteProductivity + GreenReputation + DebtRatio^2 + NetMargin + log(Asset), data=DB_Tobin, index=c("Companies", "YearFinancialIndicator"), model="within")

# M3_plm<-plm(TobinsQ ~ EnergyProductivity + CarbonProductivity + WaterProductivity + WasteProductivity + GreenReputation + DebtRatio^2 + NetMargin + log(Asset), data=DB_Tobin, index=c("Companies", "YearFinancialIndicator"), model="within")

# pooltest(M5_pvcm,M5_plm)

# pooltest(TobinsQ ~ EnergyProductivity + CarbonProductivity + WaterProductivity + WasteProductivity + GreenReputation + DebtRatio^2 + NetMargin + log(Asset), data=DB_Tobin, index=c("Companies", "YearFinancialIndicator"), model="within")
```
...
```{r}
fixed3 <- plm(TobinsQ ~ EnergyProductivity + CarbonProductivity + WaterProductivity + WasteProductivity + GreenReputation + DebtRatio^2 + NetMargin + log(Asset), data=DB_Tobin, index=c("Companies", "YearFinancialIndicator"), model="within")

random3 <- plm(TobinsQ ~ EnergyProductivity + CarbonProductivity + WaterProductivity + WasteProductivity + GreenReputation + DebtRatio^2 + NetMargin + log(Asset), data=DB_Tobin, index=c("Companies", "YearFinancialIndicator"), model = "random")
...
fixed_time3 <- plm(TobinsQ ~ EnergyProductivity + CarbonProductivity + WaterProductivity + WasteProductivity + GreenReputation + DebtRatio^2 + NetMargin + log(Asset) + factor(YearFinancialIndicator), data = DB_Tobin, index=c("Companies", "YearFinancialIndicator"), model = "within")


# Testing the time-fixed effects. The null is that no time-fixed effects needed
pFtest(fixed_time3, fixed3)


```
**Interpretation Fixed_time effect :** P-Value is > 0.05 meaning that null hypothesis is verified and that there is a non significant time-fixed effect. **So I do not need to use time fixed effect in this model!!**
...
```{r}
pcdtest(fixed_time3, test = c("lm"))
pcdtest(fixed_time3, test = c("cd"))
...
bptest(TobinsQ ~ EnergyProductivity + CarbonProductivity + WaterProductivity + WasteProductivity + GreenReputation + DebtRatio^2 + NetMargin + log(Asset) + factor(Companies), data = DB_Tobin, studentize = F)
...
```{r}
coeftest(fixed3) # Original coefficients 
coeftest(fixed3, vcovHC) # Heteroskedasticity consistent coefficients
coeftest(fixed3, vcovHC(fixed3, method = "arellano")) # Heteroskedasticity consistent
coeftest(fixed3, vcovHC(fixed3, type = "HC3")) # Heteroskedasticity consistent coefficients, type 3coefficients (Arellano)
# The following shows the HC standard errors of the coefficients
t(sapply(c("HC0", "HC1", "HC2", "HC3", "HC4"), function(x) sqrt(diag(vcovHC(fixed3, type = x)))))
```
