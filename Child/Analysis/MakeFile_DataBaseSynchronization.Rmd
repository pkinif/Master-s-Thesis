...
# I restructure each DB in removing some useless columns, renames columns, ect
  ...
  ##########################################
    ...
    # I write as FinancialData
        ...
        write.csv(DataBase2, file = "DataBase/DataSynchronization/FinancialData_1216.csv")
...
DataBase2_Lag0 <- DataBase2 %>% subset(Year < 2015)     
Db_Lag0 <- merge(DataBase2_Lag0, Lag0, by = c("Ticker", "Year"), all = TRUE) 
...
#I write as csv    
write.csv(Last, file = "DataBase/DataSynchronization/Lag0.csv") 
...
FinancialVariables_Lag1 <- DataBase2 %>%
  subset(Year > 2012) %>%
  subset(Year < 2016)

FinancialVariables_Lag0 <- DataBase2 %>%
  subset(Year < 2015)
...
write.csv(Lag1C, file = "DataBase/DataSynchronization/Lag1.csv")
...
oldDb <- read.csv2(file = "DataBase/Lag_0.csv", header = TRUE, stringsAsFactors = FALSE)
oldDb1 <- merge(Companies, oldDb, by = "Companies")
oldDb2 <- oldDb1[,c(2,9,22)]
colnames(oldDb2) <- c("Ticker", "Year", "FirmSize")

Missing <- merge(TotAsset2, oldDb2, by = c("Ticker", "Year"))

Missing$TotalAsset <- exp(Missing$FirmSize)

Missing$test <- log(Missing$TotalAsset)
write.csv(Missing, file = "DataBase/Ycharts/NoMoreMissingAsset.csv")
