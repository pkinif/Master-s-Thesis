...
In case of the absence of both RE and FE, namely \(\mu_{i} = 0\), pooled OLS estimation is the most efficient estimator [@Croissant2008]. Under FE, the random effects estimators are biased and inconsistent given that \(\mu_{i}\) is omitted and potentially correlated with other regressors. Therefore, the fixed effects estimation need to be used. Under RE, the random and fixed effects estimators are unbiased and consistent. According to @Schmidheiny2015, scholars should prefer the RE estimator only and only if \(E[\mu_{i}, X_{i}] = 0\). This precondition is tested by the Hausman test [@Hausman1981]. If H0 is verified, scholars should use the RE estimator.
