...
## Pooling Model
  CarbonPooling <- plm(CaP ~ SPL + STC + A + FirmSize + Leverage + Growth + Industry, data = Model3, model="pooling")

  WaterPooling <- plm(WaP ~ SPL + STC + A + FirmSize + Leverage + Growth + Industry, data = Model3, model="pooling")

  WastePooling <- plm(WastP ~ SPL + STC + A + FirmSize + Leverage + Growth + Industry, data = Model3, model="pooling")
...
# I test for Fixed Effect Model using pFtest which is a test of individual and/or time effects based on the comparison of the within and the pooling model.
  ...
  CarbonWithin <- plm(CaP ~ SPL + STC + A + FirmSize + Leverage + Growth + Industry, data = Model3, model="within", effect = "time")

  WaterWithin <- plm(WaP ~ SPL + STC + A + FirmSize + Leverage + Growth + Industry, data = Model3, model="within", effect = "time")

  WasteWithin <- plm(WastP ~ SPL + STC + A + FirmSize + Leverage + Growth + Industry, data = Model3, model="within", effect = "time")
...
Model (4) shows evidence of a positive and statistically significant effect of SPL, A, and WaP on *long-term CEP*. Model (5) shows evidence of a positive and statistically significant effect of SPL, STC and CaP on *short-term CEP*.
