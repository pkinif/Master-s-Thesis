...
Model1 <- pdata.frame(Model1, index = c("YearIndex", "CompaniesIndex"))
...
# I test for Fixed Effect Model using pFtest which is a test of individual and/or time effects based on the comparison of the within and the pooling model.
  ...
  CarbonWithin <- plm(CarbonProductivity ~ SustainabilityPayLink + SustainableThemedCommitment + AuditScore, data = Model3, model="within")

  WaterWithin <- plm(WaterProductivity ~ SustainabilityPayLink + SustainableThemedCommitment + AuditScore, data = Model3, model="within")

  WasteWithin <- plm(WasteProductivity ~ SustainabilityPayLink + SustainableThemedCommitment + AuditScore, data = Model3, model="within")
...
# I consolidate the p-value of the two tests into a nice stargazer table 
 ...
 # print.xtable(NiceTable1, type = "latex", table.placement = "h", caption.placement = "top", comment = FALSE, hline.after = c(-1,-1,0,3))
...
# Based on the results of the tests, the three models need to be estimated with the fixed effects estimations (i.e. model = "within" in plm). Let's consolidate into a stargazer table
  ...
  stargazer(CarbonWithin, WaterWithin, WasteWithin, title = "The impact of process-based on outcome-based CEP", label = "CepResults", header = FALSE, type = "latex", column.labels = c("Model 1", "Model 2", "Model 3"))
