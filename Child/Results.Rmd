...
StatDes <- stargazer(DB_Roa[,-c(1:2)], type = "latex", title = "Statistic Descriptive", label = "StatDes", header = FALSE)
```
...
par(mfrow=c(2,1))
boxplot(DB_Tobin$TobinsQ,horizontal=T,main="Boxplot TobinsQ")
hist(DB_Tobin$TobinsQ,horizontal=T,main="Hist TobinsQ")

par(mfrow=c(2,1))
boxplot(DB_Roa$ROA,horizontal=T,main="Boxplot ROA")
hist(DB_Roa$ROA,horizontal=T,main="Hist ROA")

par(mfrow=c(2,1))
boxplot(DB_Roa$Leverage,horizontal=T,main="Boxplot Leverage")
hist(DB_Roa$Leverage,horizontal=T,main="Hist Leverage")

par(mfrow=c(2,1))
boxplot(DB_Roa$NetMargin,horizontal=T,main="Boxplot NetMargin")
hist(DB_Roa$NetMargin,horizontal=T,main="Hist NetMargin")

par(mfrow=c(2,1))
boxplot(DB_Roa$FirmSize,horizontal=T,main="Boxplot FirmSize")
hist(DB_Roa$FirmSize,horizontal=T,main="Hist FirmSize")
```
...
## My models
M6 <-lm(ROA ~ SustainabilityPayLink + SustainableThemedCommitment + AuditScore + EnergyProductivity + CarbonProductivity + WaterProductivity + WasteProductivity + Leverage + NetMargin + FirmSize + Industry, data=DB_Roa)
M5 <-lm(TobinsQ ~ SustainabilityPayLink + SustainableThemedCommitment + AuditScore + EnergyProductivity + CarbonProductivity + WaterProductivity + WasteProductivity + Leverage + NetMargin + FirmSize + Industry, data=DB_Tobin)
